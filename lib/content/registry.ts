...
/**
 * A safe view of one scenario, without hiddenNeed, manner or resolution
 * conditions.
 *
 ...
 */
function project(artifact: ScenarioArtifact) {
  const persona = PERSONAS[artifact.personaId];
...
export type PublicScenario = ReturnType<typeof project>;

/** The card for one registered scenario, for a session that references it. */
export function publicScenario(id: string): PublicScenario | undefined {
  const artifact = SCENARIOS[id];
  return artifact ? project(artifact) : undefined;
}

/** The catalogue: every playable scenario, plus the ones only announced. */
export function listScenarios() {
  const ready = Object.values(SCENARIOS).map(project);

  // Announced but not playable: no scenario exists, so these ids are unknown to
  // getScenario and a session cannot be opened on them.
